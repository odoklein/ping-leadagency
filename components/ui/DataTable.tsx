...
export function DataTable<T extends Record<string, any>>({
    ...
    // `searchInput` is what the user sees; `searchQuery` is what we filter on.
    // Filtering walks every row, so debouncing keeps big lists responsive while typing.
    ...
    return (
        <div className={cn("space-y-0", className)}>
            {/* Toolbar: search + column visibility on a single row.
                These used to be two stacked bars, which cost vertical space
                before the first row of actual data. */}
            {(searchable || (enableSecondaryColumnsToggle && secondaryColumns.length > 0)) && (
                <div className="flex items-center gap-3 px-5 py-3 border-b border-line bg-paper/50">
                    {searchable && (
                        <div className="relative flex-1 min-w-0">
                            ...
                            <input
                                ...
                                className="w-full pl-9 pr-9 py-2 bg-surface border border-line rounded-elan text-ink text-sm placeholder:text-slate focus:outline-none focus:ring-2 focus:ring-amber/20 focus:border-amber transition-all"
                            />
                            {searchInput && (
                                <button
                                    onClick={() => setSearchInput("")}
                                    className="absolute right-2 top-1/2 -translate-y-1/2 p-1 text-slate hover:text-ink rounded-md hover:bg-paper-2 transition-colors"
                    ...
                    {enableSecondaryColumnsToggle && secondaryColumns.length > 0 && (
                        <div className="relative flex-shrink-0">
                            <button
                                ...
                                className="inline-flex items-center gap-1.5 px-3 py-2 rounded-elan border border-line bg-surface text-xs font-medium text-ink-soft hover:border-line-strong transition-colors"
                            >
                                <Columns3 className="w-3.5 h-3.5 text-slate" />
                                {visibleSecondaryKeys.length > 0
                                    ? `${visibleSecondaryKeys.length}/${secondaryColumns.length} colonnes`
                                    : `+${secondaryColumns.length} colonnes`}
                                        ...
                                        <button
                                            ...
                                            onClick={() =>
                                                setVisibleSecondaryKeys((prev) =>
                                                    prev.length === secondaryColumns.length
                                                        ? []
                                                        : secondaryColumns.map((c) => c.key)
                                                )
                                            }
                                            className="text-[11px] font-medium text-amber hover:underline"
                                        >
                                            {visibleSecondaryKeys.length === secondaryColumns.length
                                                ? "Aucune"
                                                : "Toutes"}
                                        </button>
                                    </div>
